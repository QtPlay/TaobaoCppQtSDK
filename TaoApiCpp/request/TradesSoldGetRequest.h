#ifndef TRADESSOLDGETREQUEST_H
#define TRADESSOLDGETREQUEST_H
...
/**
 * TOP API: 搜索当前会话用户作为卖家已卖出的交易数据（只能获取到三个月以内的交易信息） 
 ...
 */
class TradesSoldGetRequest : public TaoRequest
{
 public:
...
;  void setPageSize (qlonglong pageSize);
 ...
 QString getRateStatus() const
;  void setRateStatus (QString rateStatus);
...
;  void setUseHasNext (bool useHasNext);
  ...
  virtual TradesSoldGetResponse *getResponseClass(const QString &session = "",
...
/**
 * @brief 查询交易创建时间结束。格式:yyyy-MM-dd HH:mm:ss
 **/
...
/**
 * @brief 评价状态，默认查询所有评价状态的数据，除了默认值外每次只能查询一种状态。<br> 
可选值： 
RATE_UNBUYER(买家未评) 
RATE_UNSELLER(卖家未评) 
...
/**
 * @brief 查询三个月内交易创建时间开始。格式:yyyy-MM-dd HH:mm:ss
 **/
  QDateTime startCreated;

/**
 * @brief 交易状态，默认查询所有交易状态的数据，除了默认值外每次只能查询一种状态。 
可选值： 
...
/**
 * @brief 是否启用has_next的分页方式，如果指定true,则返回的结果中不包含总记录数，但是会新增一个是否存在下一页的的字段，通过此种方式获取增量交易，接口调用成功率在原有的基础上有所提升。
 **/
  bool useHasNext;

};

#endif  /* TRADESSOLDGETREQUEST_H */
