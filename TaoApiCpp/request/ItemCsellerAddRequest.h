#ifndef ITEMCSELLERADDREQUEST_H
#define ITEMCSELLERADDREQUEST_H
...
/**
 * TOP API: 淘宝助理提供的发布集市商品接口，接口参数除了包括taobao.item.vip.add的参数外，新增一个sub_pic_paths参数，为图片空间的url，本方法会在发布商品同时，将这些图片作为副图关联到新商品。
 * 
 * @author sd44 <sd44sdd44@yeah.net>
 */
class ItemCsellerAddRequest : public TaoRequest
{
 public:
...
;  void setStuffStatus (QString stuffStatus);
 ...
 QString getSubPicPaths() const
;  void setSubPicPaths (QString subPicPaths);
...
;  void setWeight (qlonglong weight);
  ...
  virtual ItemCsellerAddResponse *getResponseClass(const QString &session = "",
...
/**
 * @brief 表示商品的体积，如果需要使用按体积计费的运费模板，一定要设置这个值。该值的单位为立方米（m3），如果是其他单位，请转换成成立方米。 该值支持两种格式的设置：格式1：bulk:3,单位为立方米(m3),表示直接设置为商品的体积。格式2：length:10;breadth:10;height:10，单位为米（m）。体积和长宽高都支持小数类型。 在传入体积或长宽高时候，不能带单位。体积的单位默认为立方米（m3），长宽高的单位默认为米(m) 该值支持两种格式的设置：格式1：bulk:3,单位为立方米(m3),表示直接设置为商品的体积。格式2：length:10;breadth:10;height:10，单位为米（m）
 **/
  QString itemSize;
...
/**
 * @brief 1~4个淘宝图片空间的图片相对url列表，“,”分隔，做为商品的副图
 **/
  QString subPicPaths;
