#ifndef WLBORDERCREATEREQUEST_H
#define WLBORDERCREATEREQUEST_H
...
/**
 * TOP API: 创建物流宝订单，由外部ISV或者ERP，Elink，淘宝交易产生
 * 
 * @author sd44 <sd44sdd44@yeah.net>
 */
class WlbOrderCreateRequest : public TaoRequest
{
 public:
...
;  void setExpectStartTime (QDateTime expectStartTime);
 ...
 QString getInvoinceInfo() const
;  void setInvoinceInfo (QString invoinceInfo);
...
;  void setOrderCode (QString orderCode);
 ...
 QString getOrderFlag() const
;  void setOrderFlag (QString orderFlag);

 QString getOrderItemList() const
;  void setOrderItemList (QString orderItemList);
...
;  void setOutBizCode (QString outBizCode);
 ...
 qlonglong getPackageCount() const
;  void setPackageCount (qlonglong packageCount);

 qlonglong getPayableAmount() const
;  void setPayableAmount (qlonglong payableAmount);

 QString getPrevOrderCode() const
;  void setPrevOrderCode (QString prevOrderCode);

 QString getReceiverInfo() const
;  void setReceiverInfo (QString receiverInfo);
...
;  void setScheduleType (QString scheduleType);

 QString getSenderInfo() const
;  void setSenderInfo (QString senderInfo);
...
;  void setStoreCode (QString storeCode);
 ...
 QString getTmsInfo() const
;  void setTmsInfo (QString tmsInfo);

 QString getTmsOrderCode() const
;  void setTmsOrderCode (QString tmsOrderCode);

 QString getTmsServiceCode() const
;  void setTmsServiceCode (QString tmsServiceCode);

 qlonglong getTotalAmount() const
;  void setTotalAmount (qlonglong totalAmount);


  virtual WlbOrderCreateResponse *getResponseClass(const QString &session = "",
...
/**
 * @brief 期望结束时间，在入库单会使用到
 **/
  QDateTime expectEndTime;

/**
 * @brief 计划开始送达时间  在入库单中可能会使用
 **/
  QDateTime expectStartTime;

/**
 * @brief {"invoince_info": [{"bill_type":"发票类型，必选", "bill_title":"发票抬头，必选", "bill_amount":"发票金额(单位是分)，必选","bill_content":"发票内容，可选"}]}
 **/
  QString invoinceInfo;

/**
 * @brief 该物流宝订单是否已完成，如果完成则设置为true，如果为false，则需要等待继续创建订单商品信息。
 **/
  bool isFinished;

/**
 * @brief 物流宝订单编号，该接口约定每次最多只能传50条order_item_list，如果一个物流宝订单超过50条商品的时候，需要批量来调用该接口，第一次调用的时候，wlb_order_code为空，如果第一次创建成功，该接口返回wlb_order_code，其后继续再该订单上添加商品条目，需要带上wlb_order_code，out_biz_code，order_item_list,is_finished四个字段。
 **/
  QString orderCode;

/**
 * @brief 用字符串格式来表示订单标记列表：比如COD^PRESELL^SPLIT^LIMIT 等，中间用“^”来隔开 ---------------------------------------- 订单标记list（所有字母全部大写）： 1: COD –货到付款 2: LIMIT-限时配送 3: PRESELL-预售 5:COMPLAIN-已投诉 7:SPLIT-拆单， 8:EXCHANGE-换货， 9:VISIT-上门 ， 10: MODIFYTRANSPORT-是否可改配送方式， 
: 是否可改配送方式  默认可更改 
...
/**
 * @brief 订单商品列表： {"order_item_list":[{"trade_code":"可选,淘宝交易订单，并且不是赠品，必须要传订单来源编号"," sub_trade_code ":"可选,淘宝子交易号","item_id":"必须,商品Id","item_code":"必须,商家编码","item_name":"可选,物流宝商品名称","item_quantity":"必选,计划数量","item_price":"必选,物品价格,单位为分","owner_user_nick
...
/**
 * @brief 外部订单业务ID，该编号在isv中是唯一编号， 用来控制并发，去重用
 **/
  QString outBizCode;

/**
 * @brief 包裹件数，入库单和出库单中会用到
 **/
  qlonglong packageCount;

/**
 * @brief 应收金额，cod订单必选
 **/
  qlonglong payableAmount;
...
/**
 * @brief 投递时间范围要求,格式'15:20'用分号隔开
 **/
  QString scheduleEnd;

/**
 * @brief 投递时间范围要求,格式'13:20'用分号隔开
 **/
...
/**
 * @brief 发货方信息，发收方发货方信息必须传 手机和电话必选其一
邮编^^^省^^^市^^^区^^^具体地址^^^收件方名称^^^手机^^^电话
如果某一个字段的数据为空时，必须传NA
 **/
  QString senderInfo;
...
/**
 * @brief cod服务费，只有cod订单的时候，才需要这个字段
 **/
