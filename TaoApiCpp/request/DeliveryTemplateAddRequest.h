#ifndef DELIVERYTEMPLATEADDREQUEST_H
#define DELIVERYTEMPLATEADDREQUEST_H
 ...
 */
class DeliveryTemplateAddRequest : public TaoRequest
{
 public:
...
;  void setValuation (qlonglong valuation);
  ...
  virtual DeliveryTemplateAddResponse *getResponseClass(const QString &session = "",
...
/**
 * @brief 运费模板的名称，长度不能超过50个字节
 **/
  QString name;

/**
 * @brief 增费：输入0.00-999.99（最多包含两位小数） 
...
/**
 * @brief 增费标准：当valuation(记价方式)为0时输入1-9999范围内的整数<br/><br/><font color=red>当valuation(记价方式)为1时输入0.1-9999.9范围内的小数只能包含以为小数（单位为千克）<br/><br/><font color=blue>当valuation(记价方式)为3时输入0.1-999.9范围内的数值，数值只能包含一位小数（单位为 立方米） 
<br/> 
<br/> 
<br/> 
...
/**
 * @brief 首费：输入0.00-999.99（最多包含两位小数） 
<br/><br><font color=red>输入的格式分号个数和template_types数量一致，逗号个数必须与template_dests以分号隔开之后一一对应的数量一致</font>
 **/
  QString templateStartFees;

/**
 * @brief 首费标准：当valuation(记价方式)为0时输入1-9999范围内的整数<br/><br/><font color=red>当valuation(记价方式)为1时输入0.1-9999.9范围内的小数只能包含以为小数（单位为千克）<br/><br/><font color=blue>当valuation(记价方式)为3时输入0.1-999.9范围内的数值，数值只能包含一位小数（单位为 立方米） 
<br/> 
<br/> 
<br/> 
...
/**
 * @brief 运费方式:平邮 (post),快递公司(express),EMS (ems),货到付款(cod),物流宝保障速递(wlb),家装物流(furniture)结构:value1;value2;value3;value4 
...
/**
 * @brief 可选值：0<br>说明：<br>0:表示按宝贝件数计算运费 <br>1:表示按宝贝重量计算运费 
 <br>3:表示按宝贝体积计算运费
 **/
  qlonglong valuation;
...
#endif  /* DELIVERYTEMPLATEADDREQUEST_H */
