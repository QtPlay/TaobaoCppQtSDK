#ifndef TRADESSOLDINCREMENTGETREQUEST_H
#define TRADESSOLDINCREMENTGETREQUEST_H
 ...
 */
class TradesSoldIncrementGetRequest : public TaoRequest
{
 public:
...
;  void setUseHasNext (bool useHasNext);
  ...
  virtual TradesSoldIncrementGetResponse *getResponseClass(const QString &session = "",
                               const QString &accessToken = ""); 

 private:
/**
 * @brief 查询修改结束时间，必须大于修改开始时间(修改时间跨度不能大于一天)，格式:yyyy-MM-dd HH:mm:ss。<span style="color:red;font-weight: bold;">建议使用30分钟以内的时间跨度，能大大提高响应速度和成功率</span>。
 **/
...
/**
 * @brief 查询修改开始时间(修改时间跨度不能大于一天)。格式:yyyy-MM-dd HH:mm:ss
 **/
  QDateTime startModified;

/**
 * @brief 交易状态，默认查询所有交易状态的数据，除了默认值外每次只能查询一种状态。 可选值 TRADE_NO_CREATE_PAY(没有创建支付宝交易) WAIT_BUYER_PAY(等待买家付款) 
...
#endif  /* TRADESSOLDINCREMENTGETREQUEST_H */
